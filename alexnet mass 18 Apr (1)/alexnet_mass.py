...
data = read_csv("mass_case_train.csv")[1:]
...
for row in data:
    if row[-4] == "BENIGN" or row[-4] == "BENIGN_WITHOUT_CALLBACK":
        ...
        for i in range(6):
            y.append([0,1])
    file_path = os.path.join('Mass-Train', row[-2].replace("\\", "/"))
...
test_data = read_csv("mass_case_test.csv")[1:]
...
for row in test_data:
    if row[-4] == "BENIGN" or row[-4] == "BENIGN_WITHOUT_CALLBACK":
        ...
        test_y.append([0,1])
    file_path = os.path.join('Mass-Test', row[-2].replace("\\", "/"))
    img_array = skimage.transform.resize(skimage.io.imread(file_path), (227,227,3))
    img_array = skimage.exposure.equalize_hist(img_array)
    img_array = skimage.color.gray2rgb(img_array)
...
csv_logger = CSVLogger("metric_mass_5.csv", separator = ",", append = True)
model_history = model.fit(x = input_x, y = output_y, epochs=50, validation_data = (test_x, test_y), verbose = 2, callbacks = [csv_logger])
model.save("alexnet_mass.h5")

mass_metrics = pd.DataFrame(columns = ["loss", "acc", "val_loss", "val_acc"])
mass_metrics = mass_metrics.append(pd.DataFrame(model_history.history), ignore_index=True)
mass_metrics.to_csv(r"mass_metrics_6_pd_to_csv.csv")
